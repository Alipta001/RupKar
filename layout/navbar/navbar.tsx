...
//               className={`hidden lg:flex items-center gap-3 group cursor-pointer px-3 py-1.5 rounded-xl transition-all ${isProfileOpen ? 'bg-green-50 ring-1 ring-green-100' : 'hover:bg-green-50'}`}
//             >
...
//                   Alipta Ghosh <ChevronDown className={`w-3 h-3 transition-transform duration-300 ${isProfileOpen ? 'rotate-180' : ''}`} />
//                 </span>
//               </div>
//             </div>
...
//                 </div>
//                 <h2 className="text-xl font-black italic">RupoKar Premium</h2>
//               </div>
  ...
  return (
    <header className="w-full bg-white shadow-sm font-sans sticky top-0 z-[100]">
      ...
      <div className="hidden lg:block border-t border-gray-100">
        <div className="max-w-[1920px] mx-auto px-10 py-3">
          <ul className="flex items-center gap-10 text-sm font-bold text-gray-600">
            {['All Categories', 'Home', 'Top Deals', 'Best Sellers', 'Electronics', 'Fashion', 'Home Decor', 'New Arrivals'].map((item, idx) => (
              <li key={idx} className="flex items-center gap-1 hover:text-green-600 cursor-pointer transition-colors relative group">
                {item === 'All Categories' && <Menu className="w-4 h-4" />}
                {item}
                <span className="absolute -bottom-[13px] left-0 w-0 h-[3px] bg-green-600 transition-all group-hover:w-full rounded-t-full" />
          ...
          <>
            ...
            >
              <div className="p-6 bg-green-600 text-white">
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold opacity-80 uppercase tracking-tighter">Verified User</span>
                      <span className="font-bold text-sm">Alipta Ghosh</span>
                    </div>
                  </div>
                  <button 
                    onClick={() => setIsMobileMenuOpen(false)}
                ...
                <h2 className="text-xl font-black italic tracking-tighter">RupoKar Premium</h2>
              </div>
              
              <nav className="p-5 overflow-y-auto space-y-8">
                {/* ACCOUNT & ORDERS SECTION */}
                <div>
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">My Account</h3>
                  <ul className="space-y-5 font-bold text-gray-800 text-sm">
                    <li className="flex items-center gap-3 hover:text-green-600 cursor-pointer">
                      <User className="w-4 h-4 text-gray-400" onClick={()=> router.push("/user/profile")}/> My Profile
                    </li>
                    <li className="flex items-center gap-3 hover:text-green-600 cursor-pointer">
                ...
                <div>
                  ...
                  <ul className="space-y-5 font-bold text-gray-800 text-sm">
                    <li className="flex justify-between items-center group cursor-pointer hover:text-green-600">
                      Fashion <ChevronDown className="w-4 h-4 text-gray-300 group-hover:text-green-600" />
                    </li>
                    <li className="flex justify-between items-center group cursor-pointer hover:text-green-600">
                      Electronics <ChevronDown className="w-4 h-4 text-gray-300 group-hover:text-green-600" />
                    </li>
                    <li className="flex justify-between items-center group cursor-pointer hover:text-green-600">
                      Home Decor <ChevronDown className="w-4 h-4 text-gray-300 group-hover:text-green-600" />
                    </li>
                  </ul>
                </div>
                ...
                <div className="space-y-4">
                  <button className="w-full py-3.5 bg-green-100 text-green-700 font-black rounded-xl border-2 border-green-200 text-xs uppercase tracking-widest hover:bg-green-200 transition-all">
                    Become a Seller
                  </button>
